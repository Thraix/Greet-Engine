...
namespace Greet {
  ...
  uint32_t LogUtils::Hex10ToDec16(const char c)
  {
    if (c >= '0' && c <= '9')
      return (uint32_t)(c - '0');
    else if (c >= 'a' && c <= 'f')
  ...
  char LogUtils::Dec16ToHex10(int dec)
  {
    switch (dec)
    {
      case 0:
  ...
  std::string LogUtils::DecToHex(uint32_t dec, uint32_t values)
  {
    std::string s = "";
    bool trailing = true;
  ...
  uint32_t LogUtils::HexToDec(const std::string& hex)
  {
    if (hex.length() > 8)
    {
      return 0;
